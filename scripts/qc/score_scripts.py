...
def main(paths):
    ...
    print(f"{'scenario':46s} {'words':>5s} {'conc':>5s} {'abst':>5s} {'meta':>4s} "
          f"{'guid':>5s} {'degen':>6s} {'collap':>6s}")
    for label, s in rows:
        flag = f"{s['degen_lost']:.0%}" if s["degen"] else "-"
        # Calibrated against A_gold 2026-06-09: gold concreteness ranges 1.2-12.8
        # (style-dependent), so it's a soft flag only; degen + meta + collapsed
        # are hard flags (zero gold exemplars trip any of them).
        warn = " <-- READ ME" if (s["degen"] or s["meta"] or s["collapsed"]
                                  or s["abstract"] > 3 or s["concrete"] < 4) else ""
        print(f"{label:46s} {s['words']:5d} {s['concrete']:5.1f} {s['abstract']:5.1f} "
              f"{s['meta']:4d} {s['guided']:5.1f} {flag:>6s} {s['collapsed']:6d}{warn}")
